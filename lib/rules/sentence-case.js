...
const checkString = (context) => {
    const [options = {}] = context.options || []

    const extendedPhraseBank =
        options.phraseBank && options.phraseBank.length
            ? new Set([...phraseBank, ...options.phraseBank])
            : phraseBank
    ...
    return (node) => {
        ...
        const filteredValue = excludePhrasesFromWordBank(
            value,
            extendedPhraseBank
        )

        const lowerToUpperCount = (
            filteredValue.match(/\p{Ll}\s\p{Lu}/gu) || []
        ).length
